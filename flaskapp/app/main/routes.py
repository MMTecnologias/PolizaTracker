...
def check_access(nombre_del_servicio):
    ...
    servicio_crear_usuario_id = Servicio.query.filter_by(
        nombre=nombre_del_servicio).first().id
    acceso = Acceso.query.filter_by(
        servicio_id=servicio_crear_usuario_id, nivel_id=current_user.nivel_id).first()
    if not acceso:
        return False
    return True
...
# Reportes Fecha de nacimiento


@main.route('/reporteNacimiento', methods=['GET'])
@login_required
def reporteNacimiento():
    return render_template('reporteNacimiento.html')

# Reportes de recibos pagados


@main.route('/reporteRecibosPagados', methods=['GET'])
@login_required
def reporteRecibosPagados():
    return render_template('reporteRecibosPagados.html')
...
def create_multiple():
    ...
    form_id = request.form.get(
        'form_id') if request.form.get('form_id') else "New"
...
def load():
    ...
    bool_return, msg = load_data_from_csv(
        Cliente, 'C:/Users/luism/Downloads/clientes.csv')
    return msg
    ...
    try:
        with open(csv_file, 'r', encoding='utf-8-sig') as file:
            ...
            for row in reader:
                ...
                # Create a new record in the table using the dictionary
