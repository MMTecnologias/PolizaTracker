...
const currentPageData = async () => {
   ...
   if (sorting === true) {
      currentData = await fetch('/get_sorted_clients_data', {
         method: 'POST',
         headers: {
      ...
      currentData = await fetch('/get_clients_data2', {
         method: 'POST',
         headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
         },
   ...
   console.log(`Estoy imprimiendo desde currentPageData ${clientData}`);
   return clientData;
};
